...
from execution.test_support import IDENTITY, make_plan, tokens
...
class UltraBindingTests(unittest.TestCase):
    ...
    def test_binder_binds_only_exact_dependencies_and_copies_messages(self):
        ...
        request = bind_ultra_operation(plan, op["id"], artifacts, runtime_identity=IDENTITY, token_counter=tokens)
        self.assertEqual(plan, original)
        self.assertFalse(request["stream"])
        self.assertNotIn("stream_options", request)
        self.assertEqual(request["model"], IDENTITY["model"])
    ...
    def test_only_synthesis_can_use_the_explicitly_skipped_debate(self):
        ...
        request = bind_ultra_operation(plan, "synthesis", artifacts, runtime_identity=IDENTITY, token_counter=tokens)
        self.assertTrue(request["stream"])
        self.assertEqual(request["stream_options"], {"include_usage": True})
        artifacts["judge"] = None
        with self.assertRaises(ExecutionError):
            bind_ultra_operation(plan, "synthesis", artifacts, runtime_identity=IDENTITY, token_counter=tokens)
    ...
    def test_dependency_and_placeholder_tampering_is_rejected(self):
        ...
        for artifacts in ({}, {**good, "attacker": "injected"},
            ...
            with self.assertRaises(ExecutionError):
                bind_ultra_operation(plan, "judge", artifacts, runtime_identity=IDENTITY, token_counter=tokens)
        for change in ({"target_message_index": 0}, {"target_field": "role"},
            ...
            with self.assertRaises(ExecutionError):
                bind_ultra_operation(altered, "judge", good, runtime_identity=IDENTITY, token_counter=tokens)
    ...
    def test_recount_and_served_context_limits_are_enforced_before_dispatch(self):
        ...
        for counter in (lambda *_: True, lambda *_: 0, lambda *_: op["maximum_input_tokens"] + 1):
            with self.assertRaises(ExecutionError):
                bind_ultra_operation(plan, op["id"], {}, runtime_identity=IDENTITY, token_counter=counter)
        with self.assertRaises(ExecutionError):
            bind_ultra_operation(plan, op["id"], {}, runtime_identity={**IDENTITY, "context_tokens": 1}, token_counter=tokens)
