...
const required = [
  "benchmark_scope", "request_id", "attempt_id", "outcome", "model", "model_revision", "measurement_source",
...
function summarizeGroup(records) {
  ...
  return {
    ...
    successful_candidate_attempts: successful.length,
    failed_candidate_attempts: failed.length,
    total_attributable_compute_cost_usd: totalComputeCost,
    average_compute_cost_per_successful_candidate_attempt_usd: successful.length ? totalComputeCost / successful.length : null,
...
export function summarizeBenchmark(records) {
  ...
  for (const [index, record] of records.entries()) {
    ...
    if (record.benchmark_scope !== "isolated_model_candidate") throw new Error(`record ${index} invalid benchmark_scope`);
    for (const forbidden of ["engine", "route_id", "stage_id", "public_response", "required_operation_ids"]) {
      if (forbidden in record) throw new Error(`record ${index} route telemetry is not valid for candidate benchmark`);
    }
  ...
  return {
    ...
    ultra_route_completion_supported: false,
    use_for_customer_route_pricing: false,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!input) throw new Error("usage: npm run benchmark:candidate:summarize -- benchmark.json");
